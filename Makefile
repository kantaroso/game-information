...
build-go-base:
	$(eval TAGDATE := $(shell date "+%Y%m%d%H%M%S"))
	docker build -f ./infra/docker/go/gin/base/Dockerfile -t kantaroso/game-information-go-prod:${TAGDATE} -t kantaroso/game-information-go-base:latest .
	docker push kantaroso/game-information-go-base:latest
build-go-pord:
	$(eval TAGDATE := $(shell date "+%Y%m%d%H%M%S"))
